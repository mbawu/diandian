...
public class BaiduMapActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        String houseName = "点点";
        try{
        	String latlog = (String)getIntent().getSerializableExtra(HouseDetails.latlog);
        	houseName = (String)getIntent().getSerializableExtra("housesname");
	        if(latlog != null){
	        	String data[] = latlog.split(",");
	        	if(data.length==2){
	        		cLon = Double.parseDouble(data[0]);
	        		cLat = Double.parseDouble(data[1]);
	        	}
	        }
	        this.setTitle(houseName);
        }catch(Exception e){
      ...
      //生成ItemizedOverlay图层用来标注结果点
		...
		//生成Item
		OverlayItem item = new OverlayItem(p, houseName, null);
		//得到需要标在地图上的资源
